...
namespace OnvifModule
{
    public class ProgramLauncher
    {
        ...
        //Edge gateway template model id
        private static string ModelId = Program.SecretDic["EDGE_GATEWAY_MODULE_ID"];//Environment.GetEnvironmentVariable("EDGE_GATEWAY_MODULE_ID");


        private static ILogger s_logger;

        public static async Task CreateGateController()
        {
            // Parse application parameters
            Parameters parameters = new Parameters();
            parameters.DeviceSecurityType = "DPS";
            parameters.DpsEndpoint = "global.azure-devices-provisioning.net";
            parameters.DpsIdScope = Program.SecretDic["EDGE_GATEWAY_SCOPE_ID"];
            parameters.DeviceId = Program.SecretDic["EDGE_GATEWAY_DEVICE_ID"];
            string groupSASKey = Program.SecretDic["EDGE_GATEWAY_GROUP_SAS_KEY"];
            ...
            var gatewayController = new GatewayController(deviceClient, s_logger);         
            await gatewayController.PerformGatewayOperationsAsync(cts.Token);
            //await gatewayController.PerformOperationsAsync(cts.Token);
